...
uint32_t initChannelSettingCnt = 0;

struct channelSettingParam_s channelSettings[MIDI_CH_CNT];

struct channelSettingParam_s *currentChSetting = &channelSettings[0];

uint8_t selectedOp = OP4;
...
{
    //channelSettings->velocity_to_lvl = 0;
...
void FmSynth_AlgMixProcess(float *left, struct synthVoice_s *voice)
{
    voice->out = 0.0f;
...
void FmSynth_Process(float *left, int bufLen)
{
    ...
    for (int n = 0; n < bufLen; n++)
    {
        ...
        for (int j = 0; j < FM_VOICE_CNT; j++)
        {
            ...
            FmSynth_AlgMixProcess(&left[n], voice);
        }
    }
    ...
    if (initChannelSetting)
    {
        ...
        if (initChannelSettingCnt > 3 * ((float)SAMPLE_RATE / (float)SAMPLE_BUFFER_SIZE))
        {
...
void FmSynth_Decay2(uint8_t unused, float value)
{
    ...
    //Serial.printf("decay2: %0.3f\n",  currentChSetting->op_prop[selectedOp].d2r);
}
