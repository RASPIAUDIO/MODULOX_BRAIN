...
public:
  ...
  inline int16_t process_grain(Grain &g, int pol) {
	...
	if(index>=param_end_index[active_sample]) index-=param_end_index[active_sample];
  ...
  float process() {
    ...
    for (int j = 0; j < POLYPHONY; j++) {
      ...
      if (notepressed[j].isplaying) {
		...
		if (sample_pos[j] >= param_end_index[active_sample]) {
			...
			sample_pos[j] = param_start_index[active_sample];
