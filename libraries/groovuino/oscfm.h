...
#define NUM_OSC 6  // Change this to change the number of oscillators per voice

// Can adjust the Sample rate
#define SAMPLE_RATE 44100.0 
#define ENVTIME (1.0/(float)SAMPLE_RATE)
...
public:
	...
	float voldesc;
	float accent;
	uint8_t EGrate[NUM_OSC][4];
	uint8_t EGlevel[NUM_OSC][4];
	float EGval[NUM_OSC];
...
// Set the waveform of one oscillator
   void setWaveform(uint8_t val)
   {
    ...
    void envnext(int num)
	{
		EGval[num]+=EGinc[num];
		if(phase[num]==0) // entre 0 et 30s
		{
			if(EGval[num]>=EGlevel[num][0]) 
			{
				phase[num]++;
				EGinc[num]=(EGlevel[num][1]-EGlevel[num][0])/(EGrate[num][1]+1)*30*ENVTIME; 
			}
		}
		if(phase[num]==1) // entre 0 et 30s
		{
			if(EGval[num]>=EGlevel[num][1]) 
			{
				phase[num]++;
				EGinc[num]=(EGlevel[num][2]-EGlevel[num][1])/(EGrate[num][2]+1)*30*ENVTIME; 
			}
		}
	}
